...
public class MemberDAO {
	...
	private MemberDAO() {
		...
		try {
			...
			factory = new SqlSessionFactoryBuilder().build(in);	// 팩토리를 만들어주는 건 SqlSessionFactoryBuilder() 이다.
		} catch(Exception e) {
			e.printStackTrace();
		}
	...
	private final String NS = "mybatis.mapper.member.";	// 쿼리문을 가져올 때 mapper와 "." 구분자를 이용해 쿼리문을 가져와야 하기 때문에 마지막에 "."을 붙인다.
	...
	public Member selectMemberByNo(int memberNo) {
		SqlSession ss = factory.openSession();
		// 결과가 한 줄이기 떄문에 selectOne 메소드 사용
	...
	// 삭제
	public int deleteMember(int member) {
		SqlSession ss = factory.openSession(false);
		int deleteResult = ss.delete(NS + "deleteMember", member);
		if(deleteResult == 1) {
			ss.commit();
		}
		ss.close();

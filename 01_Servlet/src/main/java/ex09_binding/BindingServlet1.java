		  	...
		  	1. stateless
		  	 ...
		  	 2. binding
		  	 	...
		  	 	3) 3개 영역
		  	 		...
		  	 		(3) ServletContext		:	컨텍스트(프로젝트, 애플리케이션(서비스관점) 종료 전까지 값을 저장할 수 있다.
		  	 	4) 속성(Attribute) 사용 관련 메소드
		  	 		(1) getAttribute("속성")		: 값 가져오기 (캐스팅이 필요할 수 있다. (Object로 저장되어 있기 때문이다.))
		  	 		(2) setAttribute("속성", '값)	: 값 저장하기 (Object 타입으로 저장한다.)
		  	 		(3) removeAttribute("속성")		: 값 제거하기
		 */
		
		// HttpServletRequest에 저장(binding)하기
		request.setAttribute("a", 100);
		
		// HttpSession에 저장(binding)하기
		HttpSession session = request.getSession();
		session.setAttribute("a", 200);
		
		// ServletContext에 저장(binding)하기

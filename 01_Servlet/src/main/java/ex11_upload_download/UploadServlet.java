...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
			...
			3. 압축 해제 후 lib(라이브러리) 디렉터리에 있는 cos.jar 파일을 프로젝트의 lib(라이브러리)로 등록한다.
		*/
		...
		*/
		...
		MultipartRequest  multipartRequest = new MultipartRequest(
				...
				realPath,			// 업로드 경로 전달
				1024 * 1024 * 10,	// 최대 크기 전달
				"UTF-8",			// 인코딩
				new DefaultFileRenamePolicy()	// 파일명 중복 처리 정책 (파일명 뒤에 넘버링)
				);
				
		// 요청 파라미터 (MultipartRequest mulripartRequest를 이용해서 파라미터를 가져와야 한다.)
		...
		String size = new DecimalFormat("#,##0").format(file.length() / 1024 + (file.length() % 1024 == 0 ? 0 : 1));	// 3자리마다 "," 붙여준다
		...
		out.println("<li>원래 첨부파일명 : " + originName + "</li>");

...
public class CookieServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
			...
			1. 서버가 만들어서 클라이언트가 저장한다. (기록을 남김)
			2. 보안에 취약하다. (중요한 정보를 보관하지 않는다.)
		*/

		// 쿠키 만들기
		Cookie cookie1 = new Cookie("name", "김동형");	// 동일한 경로에 동일한 이름이 있다면 덮어쓰기 된다.
		...
		cookie1.setPath("/01_Servlet");					// 컨텍스트 패스 : (/01_Servlet)request.getContextPath()
		cookie2.setPath("/01_Servlet/CookieServlet"); 	// 전체 경로(서블릿 경로) : (request.getRequestURI())
														// cookie3은 경로 설정을 생략했으므로 컨텍스트패스에 저장된다.
		
		// 쿠키 유효시간 설정하기	(생략하면 세션쿠키가 된다. : 브라우저를(Session) 닫을 때 까지 보관된다.)
		cookie1.setMaxAge(60 * 60 * 24 * 7);	// 7일간 보관되는 쿠키
		cookie2.setMaxAge(60 * 60);				// 1시간동안 보관되는 쿠키
												// cookie3은 유효시간 설정을 생략했으므로 세션쿠키가 된다.
		// 삭제할 땐 유효시간을 0으로 주면서 덮어쓰기가 되어 삭제가 된다.
		...
		response.sendRedirect("/01_Servlet/CookieServlet2");	// 경로가 달라질 때마다 쿠키가 달라진다.

...
public class BoardDAO {
	...
	private BoardDAO() {
		...
		try {
			Context context = new InitialContext();		// context.xml 만들기
			Context envContext = (Context)context.lookup("java:comp/env");	// Tomcat 환경에context를 읽을 준비
			dataSource = (DataSource)envContext.lookup("jdbc/GDJ61");					// name을 직접 가져옴
			/*
	...
	// 자원(Connection, PreparedStatement, ResultSet) 반납하는 메소드 만들기
	private void close() {
		try {
			if(rs != null) rs.close();
			if(ps != null) ps.close();
	...
	public List<BoardDTO> selectBoardList(){
		...
		try {
			...
			// SELECT문 실행은 executeQurey() 메소드가 실행한다.
			rs = ps.executeQuery();
			
			
			// 6. ResultSet 객체(결과 집합)를 이용해서 ArrayList를 만들어준다.
			while(rs.next()) {
				...
				// Step 2. 읽은 정보를 이용해서 BoardDTO 객체를 만든다.
		...
		public BoardDTO selectBoardByNo(int board_no) {
			...
			try {
				...
				// 6. PreparedStatement 객체를 이용해 쿼리문 실행하기
				// SELECT문 실행은 executeQurey() 메소드가 실행한다.
				rs = ps.executeQuery();
				
				
				// 7. ResultSet 객체(결과 집합)를 이용해서 BoardDTO를 만들어준다.
				if(rs.next()) {
					// Step 1. Board 테이블의 결과 행(ROW)을 읽는다.
					String title = rs.getString("TITLE");
					String content = rs.getString("CONTENT");
